...
\begin{figure}[t]
\centerline{\includegraphics[width=0.72\columnwidth]{figures/distribucion_clases.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/comparacion_modelos.png}}
...
Partiendo del mejor modelo (SVM RBF), se evaluaron varias estrategias, todas con la misma validaci\'on cruzada. La reducci\'on de dimensionalidad con PCA (30 componentes que explican el 95\% de la varianza) y con LDA no mejor\'o el F1 (0.336 y 0.210, frente a 0.339 con los 40 MFCC); la ponderaci\'on de clases (\texttt{class\_weight=balanced}) tampoco (0.322), ni los deltas de MFCC de primer y segundo orden (120 caracter\'isticas, F1 = 0.233). En cambio, enriquecer el vector con descriptores espectrales de librosa (contraste espectral, tasa de cruces por cero, centroide, ancho de banda, rolloff y energ\'ia RMS, resumidos con media y desviaci\'on est\'andar) elev\'o el F1 de 0.339 a 0.375 (64 caracter\'isticas) y se incorpor\'o al modelo final. La Tabla~\ref{tab:mejoras} resume estas estrategias.
...
Estrategia & F1 \\
\hline
MFCC (40), l\'inea base            & 0.339 \\
PCA (30 componentes)             & 0.336 \\
LDA (1 componente)               & 0.210 \\
Ponderaci\'on de clases           & 0.322 \\
MFCC + deltas (120)              & 0.233 \\
MFCC + espectral (64)            & \textbf{0.375} \\
\hline
\end{tabular}
\end{table}

La estrategia efectiva fue el ajuste del umbral de decisi\'on. Los modelos probabil\'isticos deciden por defecto con el umbral 0.5; bajo desbalance, ese valor no es \'optimo para detectar la clase minoritaria. Se obtuvieron las probabilidades fuera de muestra con validaci\'on cruzada y se seleccion\'o el umbral que maximiza el F1 de la clase positiva (Fig.~\ref{fig:umbral}), resultando en 0.33 sobre el vector enriquecido.

\begin{figure}[t]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/umbral.png}}
\caption{F1 de la clase positiva en funci\'on del umbral de decisi\'on (validaci\'on cruzada). El \'optimo (0.33) mejora claramente respecto al umbral por defecto 0.5.}
\label{fig:umbral}
\end{figure}

\subsection{Evaluaci\'on final en el conjunto de prueba}
El modelo final (SVM RBF, $C=10$, sobre las 64 caracter\'isticas y con umbral 0.33) se entrena con todo el conjunto de entrenamiento y se eval\'ua una \'unica vez sobre el conjunto de prueba. La Tabla~\ref{tab:test} compara la l\'inea base (SVM sobre 40 MFCC con umbral 0.5) con el modelo final. La combinaci\'on de las caracter\'isticas espectrales y el ajuste del umbral eleva el recall de positivos de 0.417 a 0.604, el F1 de 0.533 a 0.690 y la exactitud balanceada de 0.694 a 0.788, aumentando adem\'as la exactitud global de 0.879 a 0.910.

\begin{table}[t]
\caption{Resultados en el conjunto de prueba (m\'etricas de la clase positiva). L\'inea base: SVM sobre 40 MFCC con umbral 0.5; final: SVM sobre 64 caracter\'isticas con umbral 0.33.}
\label{tab:test}
\centering\footnotesize
...
\hline
L\'inea base (40 MFCC) & 0.879 & 0.694 & 0.741 & 0.417 & 0.533 \\
Final (64 feats)       & \textbf{0.910} & \textbf{0.788} & \textbf{0.806} & \textbf{0.604} & \textbf{0.690} \\
\hline
\end{tabular}
\end{table}

La matriz de confusi\'on del modelo final (Fig.~\ref{fig:cm}) muestra que se detectan 29 de los 48 casos positivos (60.4\%) y se clasifican correctamente 235 de los 242 negativos (97.1\%). La exactitud total es de 0.910. El sistema es, por tanto, m\'as \'util para descartar la enfermedad que para confirmarla, lo cual es coherente con la dificultad de la tarea y con el uso exclusivo del sonido.

\begin{figure}[t]
\centerline{\includegraphics[width=0.8\columnwidth]{figures/matriz_confusion_test.png}}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/pipeline_despliegue.png}}
...
De las estrategias de mejora estudiadas, la reducci\'on de dimensionalidad, la ponderaci\'on de clases y los deltas de MFCC no aportaron mejoras en validaci\'on cruzada; en cambio, enriquecer el vector con descriptores espectrales y ajustar el umbral de decisi\'on s\'i mejoraron de forma consistente la detecci\'on de positivos, elevando el F1 de la clase positiva de 0.533 a 0.690 en el conjunto de prueba. El principal factor limitante es el desbalance del conjunto y el uso exclusivo del sonido; como trabajo futuro, disponer de m\'as grabaciones positivas o incorporar informaci\'on ac\'ustica adicional podr\'ia mejorar los resultados. Con ese prop\'osito, la aplicaci\'on web desarrollada recolecta nuevas grabaciones con consentimiento. Todo el proyecto emplea una semilla fija (\texttt{SEED = 42}) y separa los conjuntos a nivel de paciente, por lo que los resultados son reproducibles. El c\'odigo fuente completo, incluido el notebook, est\'a disponible en el repositorio p\'ublico \url{https://github.com/erosas-utec/ml-project-classification}.
